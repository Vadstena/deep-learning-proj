...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Implement me!
        self.hl1 = nn.Linear(n_features, hidden_size, bias=True)    # hidden layer number does not vary
        self.hl2 = nn.Linear(hidden_size, hidden_size, bias=True)
        self.output_layer = nn.Linear(hidden_size, n_classes, bias=True)
    ...
    def forward(self, x, **kwargs):
        ...
        """
        activ_fn = F.relu      # 'activation_type' - only ReLU is needed, 'tanh' is never used
        ...
        # out = F.dropout(out, p=self.dropout) # worse results when dropout was applied to output layer
...
def main():
    ...
    for ii in epochs:
        ...
        epoch_train_losses = []
        #print("SIZE DL ", len(train_dataloader))
