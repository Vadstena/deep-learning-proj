...
class CNN(nn.Module):
    ...
    def __init__(self, dropout_prob, no_maxpool=False):
        ...
        self.ln2 = nn.Linear(320, 120)
        self.ln3 = nn.Linear(120, 4)
        if no_maxpool:
            self.c1 = nn.Conv2d(1, 8, kernel_size=3, stride=2, padding=1)
            self.c2 = nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=0)
            self.ln1 = nn.Linear(576, 320)  
        else:
            self.c1 = nn.Conv2d(1, 8, kernel_size=3)
            self.c2 = nn.Conv2d(8, 16, kernel_size=3)
            self.ln1 = nn.Linear(400, 320)  
        
    def forward(self, x):
        x = F.relu(self.c1(x.view(x.shape[0], 1, 28 , 28)))
        if not self.no_maxpool:
            x = F.max_pool2d(x, 2)
        x = F.relu(self.c2(x))
        if not self.no_maxpool:
            x = F.max_pool2d(x, 2).view(-1, 400)
        else:
            x = x.view(-1, 576)
        x = F.relu(self.ln1(x))
        x = self.drop(x)
